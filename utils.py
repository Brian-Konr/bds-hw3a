search_messages = [
    '🔍 Finding the perfect match just for you...',
    '🐾 Following the trail of your keyword...',
    '🚀 Launching the search rocket...',
    '🎈 Floating through the data clouds...',
    '🌈 Riding the data rainbow...',
    '🕵️‍♀️ On a secret mission to find your data...',
    '🎁 Unwrapping the gift of data...',
    '🔮 Gazing into the data crystal ball...',
    '🎉 It’s a data party in here...',
    '🍀 May the odds be ever in your keyword’s favor...',
    '👀 Peeking behind the data curtain...',
]